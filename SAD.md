...
**PRD:** [PRD — Система GPS-трекинга курьеров](PRD.md)
...
- **Tracks сервис** — владелец данных треков (`locations`) во всех тирах. Потребляет GPS-точки из Kafka, пишет в PostgreSQL (hot), публикует последнюю координату в Kafka. Отдаёт треки через внутренний API: hot/cold-роутинг и Redis-кэш треков — внутри сервиса
- **Couriers сервис** — профили курьеров, статусы, смены, заказы. Операторский фасад: за историческим треком ходит в Tracks API (не в его хранилища напрямую)

**Хранилища:**

- **Redis Geo** — текущие позиции курьеров (GEOADD/GEOSEARCH), hot data для карты
- **Redis Cache** — кэш завершённых треков (STRING, TTL 1 день). Принадлежит Tracks сервису (track-домен)
...
- **acks=1** — лидер партиции подтвердил запись. Компромисс: скорость при 20k RPS, потеря единичных точек при падении лидера до репликации допустима — producer получит OK, но сообщение не доедет до реплик. Клиент не узнает о потере, но единичная точка из ~5000 в треке незаметна
...
**Write path** (зелёный) — GPS-точки от курьера через Location Ingester попадают в Kafka (`raw-locations`). Мобильное приложение передаёт `shiftId` в каждом запросе (получен при открытии смены) — привязка точки к смене происходит на клиенте, серверные сервисы не делают lookup. Далее два независимых consumer group'а обрабатывают данные параллельно:
...
**Read path** (синий) — запросы диспетчера через API Gateway к Couriers сервису:

- **Карта** — Couriers читает Redis GEOSEARCH (своя read-model, наполняется из `last-coordinate`), polling каждые 5 сек
- **Трек смены** — Couriers берёт окно смены (`started_at/ended_at`) из своей БД и запрашивает точки у Tracks сервиса по внутреннему API (`shiftId` + временно́е окно). В хранилища треков напрямую не ходит — это устраняет shared-DB и оставляет владение схемой `locations` за Tracks

Hot/cold-роутинг живёт внутри Tracks (он владелец тиров и знает живость своих партиций):

- **Hot (< 1 мес)** — Redis Cache (hit) или PostgreSQL slave (miss → кэш)
- **Cold (1–12 мес)** — BigQuery

Два адаптера за единым интерфейсом. Hot window фактически 1-2 месяца (помесячные партиции — данные за начало месяца живут до дропа в следующем). Если смена старше последней живой партиции — роутинг в BigQuery.
...
- `/api/operator/shifts/{shiftId}/locations` обслуживается Couriers-фасадом, но за точками проксирует во внутренний API Tracks (владельца `locations`); сам Couriers отдаёт только метаданные смены
- Трек привязан к смене, не к курьеру (shift_id уникален глобально)
- Трек заказа = фильтрация locations по `shift_id + client_timestamp BETWEEN orders.started_at AND orders.ended_at` (shift_id берётся из таблицы orders, используется существующий индекс)
...
- Трек заказа = фильтрация locations по `shift_id + client_timestamp BETWEEN orders.started_at AND orders.ended_at` (shift_id берётся из таблицы orders, используется существующий индекс)
- Интеграция с системой заказов (этап 2): система заказов публикует события ("заказ назначен", "заказ завершён") в Kafka с `courier_id`. Couriers сервис потребляет событие и самостоятельно определяет `shift_id` по текущей открытой смене курьера — система заказов не знает про смены. При недоступности системы заказов — данные доедут с задержкой, диспетчер видит всё что успело приехать
...
| | client_timestamp | ts with tz | no | '2026-01-01T10:05:30+03:00' |

- Владелец схемы `locations` во всех тирах (hot PG, cold BigQuery, Redis-кэш) — **Tracks сервис**. Couriers к этим хранилищам напрямую не обращается, только через внутренний API Tracks — database-per-service соблюдён
...
- Партиционирование по `client_timestamp` (помесячно)
- BRIN-индекс на `client_timestamp` — данные записываются последовательно во времени, BRIN хранит min/max по блокам страниц вместо индексации каждой строки. Размер индекса ~МБ вместо десятков ГБ (B-tree) при 1.3 млрд строк/сутки. Используется для мониторинговых и отладочных запросов по диапазону времени ("сколько точек за последние 5 минут", "точки курьера за последний час"). Основные запросы треков идут через B-tree `(shift_id, client_timestamp)`
- B-tree индекс на `(shift_id, client_timestamp)` — для запросов трека конкретной смены. Составной индекс позволяет отдать точки уже отсортированными по времени без дополнительного Sort в плане запроса. Partition pruning по `shift_id` невозможен (партиции по `client_timestamp`), поэтому PG выполняет index scan в каждой живой партиции. При 2-3 живых партициях это допустимо — в нерелевантных партициях индекс возвращает 0 строк мгновенно. Для оптимизации Couriers сервис добавляет `client_timestamp BETWEEN shift.started_at AND shift.ended_at` — это позволяет планировщику отсечь лишние партиции

**BigQuery (cold storage):**

- Та же структура `locations`, данные поступают параллельно из Kafka через BigQuery Sink Connector
- `PARTITION BY DATE(client_timestamp)` — partition pruning по дате, запрос сканирует только нужный день
- `CLUSTER BY shift_id, courier_id` — данные внутри партиции отсортированы по shift_id, запрос трека смены сканирует минимум блоков. BigQuery on-demand: $6.25/ТБ scanned. Без оптимизации full scan 95 ТБ/год = ~$594/запрос. С partition pruning (1 день) = ~260 ГБ → ~$1.63. С кластеризацией поверх — сканируется только нужный shift_id, типично < $0.01/запрос
...
- Key: `track:{shiftId}`, Value: JSON-массив точек (`[{lat, lng, accuracy, client_timestamp}, ...]`), TTL 1 день
- Владелец — Tracks сервис: пишет на cache miss, читает на hit. Couriers к кэшу не обращается

### 4.6. Кэширование

- **Redis Geo** — текущие позиции всех активных курьеров. Источник данных для карты диспетчера.
- **Redis STRING** — кэш завершённых треков. TTL 1 день. Ключ: `track:{shiftId}`. Иммутабельные данные — инвалидация не нужна. Владелец — Tracks сервис.
